...
public class WordSearch {
    ...
    public void solve() throws InterruptedException
    {
        ...
        int plus = NUM_PUZZLES%NUM_THREADS;
        int diff = NUM_PUZZLES/NUM_THREADS;
        int realThreadNum = NUM_THREADS;

        if (diff == 0)
            realThreadNum = NUM_PUZZLES;

        for (int i = 0; i < realThreadNum; i++)
        {
            final int threadNumber = i;
            final int start = threadNumber * diff;
            final int end = (threadNumber + 1) * diff;

            if (i == (realThreadNum - 1))
            {
                Thread b = new Thread(() -> solve(threadNumber, start, end + plus));
                b.start();
                b.join();
            }
            ...
            {
                Thread a = new Thread(() -> solve(threadNumber, start, end));
                a.start();
                a.join();
            }
